...
from demos.fluid.common.billboard_renderer import BillboardRenderer, configure_nvidia_prime_render_offload
from demos.fluid.MMPBF.render_opengl import PlaybackScheduler, register_keyboard_controls, reverse_gravity, rotate_gravity
...
from demos.fluid.PBF2WayCoupling.simulation import (
    add_simulation_arguments, config_from_args, create_pbf2way_simulation, positive_float, run_headless,
)
...
def light_matrix(center):
    eye = np.asarray(center) + np.array([5.,8.,6.])
...
class CouplingRenderer(BillboardRenderer):
    ...
    def __init__(self, device=None, hidden=False):
        if device is not None and not wp.get_device(device).is_cuda:
            raise ValueError("The billboard frontend requires CUDA/OpenGL; use --headless for CPU simulation")
        configure_nvidia_prime_render_offload()
        super().__init__(title="Warp PBF · Two-way coupling", scaling=1.0, fps=120,
                         screen_width=1280, screen_height=800, near_plane=.05, far_plane=50,
                         camera_pos=(3.1,2.8,6.2), camera_front=(-.40,-.19,-.90),
                         background_color=(.025,.04,.07), draw_grid=False, draw_sky=False,
                         draw_axis=False, vsync=True, device=device, headless=hidden)
        from pyglet.graphics.shader import Shader, ShaderProgram
        self.mesh_program = ShaderProgram(Shader(MESH_VERTEX, "vertex"), Shader(MESH_FRAGMENT, "fragment"))
        self.depth_program = ShaderProgram(Shader(DEPTH_VERTEX, "vertex"), Shader(DEPTH_FRAGMENT, "fragment"))
        self.mesh_buffers = []
        self.poses = []
        self.colors = []
        self.light_space = light_matrix([0., 2., 0.])
        self.shadows = True
    ...
    def _create_shadow_map(self):
        ...
        gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_DEPTH_COMPONENT24, self.shadow_size, self.shadow_size,
    ...
    def _mesh(self, vertices, faces):
        ...
        data = np.concatenate((triangles, np.repeat(normal[:,None,:], 3, axis=1)), axis=2).reshape(-1,6).astype(np.float32)
        ...
        for index in (0,1):
            ...
            gl.glVertexAttribPointer(index, 3, gl.GL_FLOAT, gl.GL_FALSE, 24, ctypes.c_void_p(index*12))
        gl.glBindVertexArray(0)
        return vao, vbo, len(data)

    def update_rigid_scene(self, simulation):
        if not self.mesh_buffers:
            self.visible_bodies = [i for i,b in enumerate(simulation.body_models) if not b.wall]
            ...
            lo, hi = simulation.container_min, simulation.container_max
            # A visible receiving floor; walls stay in the physics model.
            floor = [[lo[0],lo[1]-.003,lo[2]],[hi[0],lo[1]-.003,lo[2]],
                     [hi[0],lo[1]-.003,hi[2]],[lo[0],lo[1]-.003,hi[2]]]
            self.mesh_buffers.append(self._mesh(floor, np.array([[0,2,1],[0,3,2]])))
            self.colors.append((.25,.30,.36))
    ...
    def _matrix(self, program, name, matrix, column_major=False):
        ...
        gl.glUniformMatrix4fv(gl.glGetUniformLocation(program.id, name.encode()), 1, gl.GL_FALSE,
    ...
    def screenshot(self, path):
        ...
        gl.glReadPixels(0, 0, self.screen_width, self.screen_height, gl.GL_RGB, gl.GL_UNSIGNED_BYTE, pixels)
        gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, 0)
        Path(path).parent.mkdir(parents=True, exist_ok=True)
        with Path(path).open("wb") as stream:
            pyglet.image.ImageData(self.screen_width, self.screen_height, "RGB", bytes(pixels)).save(str(path),file=stream)
...
def main():
    ...
    parser.add_argument("--playback-speed", type=positive_float, default=.5)
    parser.add_argument("--headless", action="store_true", help="Run physics without constructing OpenGL")
    parser.add_argument("--hidden", action="store_true", help="Hidden GL window for screenshot verification")
    parser.add_argument("--seconds", type=positive_float, default=10)
    parser.add_argument("--warmup", type=nonnegative_float, default=0, help="Simulate this many seconds before the first frame")
    ...
    register_keyboard_controls(renderer, on_reset=request_reset,
