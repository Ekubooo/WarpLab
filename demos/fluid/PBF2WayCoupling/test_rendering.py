...
from .render_opengl import CouplingRenderer, register_keyboard_controls, reverse_gravity, rotate_gravity
...
class RenderingTest(unittest.TestCase):
    def test_shadow_map_billboards_depth_and_controls(self):
        ...
        try:
            ...
            fluid_delta = np.abs(pixels(output/"initial-scene.png")-pixels(output/"shadow-on.png"))
            self.assertGreater(np.count_nonzero(fluid_delta>10),10000)
            self.assertEqual(renderer.gl.glGetError(),renderer.gl.GL_NO_ERROR)
            events = []
            callback = register_keyboard_controls(renderer,on_reset=lambda:events.append("reset"),
            ...
            print(f"GL verified: {np.count_nonzero(delta>10)} shadow channel samples; screenshots: {output}")
        finally:
